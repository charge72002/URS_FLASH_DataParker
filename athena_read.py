...
class athdf(dict):
  ...
  def __init__(self, filename, data=None, quantities=None, dtype=np.float32, level=None,
    ...
    with h5py.File(filename, 'r') as f:
      ...
      tsarr          = f.attrs['VariableNames'][:]
      var_quantities = [ts.decode("utf-8") for ts in tsarr]
      dmpInd    = int(filename.rfind("."))
      #print(dmpInd)
      dmpString = filename[dmpInd-5:dmpInd]
      print("[athena_read.athdf]: ",var_quantities,dmpString)
      #var_quantities = (f.attrs['VariableNames'][:]).decode("utf-8")
      ...
      tsarr                    = f.attrs['DatasetNames'][:]
      dataset_names            = np.array([ts.decode("utf-8") for ts in tsarr])
      #dataset_names            = f.attrs['DatasetNames'][:]
      dataset_sizes            = f.attrs['NumVariables'][:]
      dataset_sizes_cumulative = np.cumsum(dataset_sizes)
      tsarr                    = f.attrs['VariableNames'][:]
      variable_names           = np.array([ts.decode("utf-8") for ts in tsarr])
