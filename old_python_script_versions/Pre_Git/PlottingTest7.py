...
"""Reads HDF5 file and returns a dictionary with data."""
...
#Use this if running through Ubuntu command line
...
#Use this if running through Spyder
...
bounds = np.linspace(densityArray.min(), densityArray.max(), num=50)
#bounds = np.logspace(densityArray.min(), densityArray.max(), num=50)
#bounds = np.geomspace(densityArray.min(), densityArray.max(), num=50)
plt.tricontourf(posXarray, posYarray, densityArray, levels=50,
                norm=colors.LogNorm(vmin=densityArray.min()),
                vmax=densityArray.max())
                #norm = colors.BoundaryNorm(boundaries=bounds, ncolors = 50)
                # cmap=colors.Colormap('viridis', N=50)

# plt.pcolormesh(posXarray, posYarray, densityArray, cmap='viridis', vmin=-np.max(densityArray))
