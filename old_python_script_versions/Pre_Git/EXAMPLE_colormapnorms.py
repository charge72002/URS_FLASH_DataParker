...
#experimenting
plt.pcolormesh(X, Y, Z, cmap='viridis', vmin=-np.max(Z))
fig.colorbar(pcm, ax=ax[2], extend='both', orientation='vertical')
plt.show()
plt.tricontourf(X, Y, Z, cmap='viridis', levels = 50)
fig.colorbar(pcm, ax=ax[2], extend='both', orientation='vertical')
plt.show()
...
#ax.pcolormesh(x, y, z)
# plt.pcolormesh(longitude, latitude, topo, norm=colors.TwoSlopeNorm(vmin=-500., vcenter=0, vmax=4000))
# plt.show()
